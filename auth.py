...
class LoginHandler(BaseHandler, weiboauth.WeiboMixin):
    """
    ...
    def get(self):
        if self.get_argument("start_weibo_oauth", None):
            ...
            content += ('<h2>1. weibo Login</h2>'
            + '<form action="/login" method="get">'
            + '<input type="hidden" name="start_weibo_oauth" value="1">'
            + '<input type="submit" class="btn" value="Sign in with weibo">'
            + '</form>')
            content += ('<h2>2. Direct Login</h2>'
            + '<form class="form-inline" action="/login" method="get"> '
            + '<input type="hidden" name="start_direct_auth" value="1">'
    ...
    def _on_auth(self, user):
        ...
        if not user:
            ...
            + '<p>This might be due to a problem in Tornados authentication.</p>'
            + '</div>')
            self.render_default("index.html", content=content)
            return None
